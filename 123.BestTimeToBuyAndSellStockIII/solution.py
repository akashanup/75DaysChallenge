    ...
    def dp(self, prices, day, transactions, stocksInHand, lookup):
        ...
        if key not in lookup:
            if stocksInHand:
                ...
                lookup[key] = max(self.dp(prices, day + 1, transactions, True, lookup),
                                  prices[day] + self.dp(prices, day + 1, transactions + 1, False, lookup))
            else:
                # I can either buy the stocks or do nothing.
                lookup[key] = max(-prices[day] + self.dp(prices, day + 1, transactions, True, lookup),
                                  self.dp(prices, day + 1, transactions, False, lookup))
        return lookup[key]
